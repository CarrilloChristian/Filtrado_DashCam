...
#====================================================================================
# Si la velocidad promedio es menor a este umbral, se considera una maniobra
# a baja velocidad y, por lo tanto, un falso positivo.
VELOCIDAD_UMBRAL = 18
...
def _extraer_velocidades(cap):
    ...
    saltando fotogramas para mayor velocidad.
    """
...
def analizar(cap):
    """
    Analiza un video de distracción del conductor. Devuelve True si es una alerta real.
    """
    lista_velocidades = _extraer_velocidades(cap)
    
    if not lista_velocidades:
        return False # Datos insuficientes = Falso Positivo
        
    velocidad_promedio = sum(lista_velocidades) / len(lista_velocidades)
    
    if velocidad_promedio < VELOCIDAD_UMBRAL:
        return False # Falso Positivo (maniobra a baja velocidad)
    else:
        return True # Positivo Real (distracción a alta velocidad)